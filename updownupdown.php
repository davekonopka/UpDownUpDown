...
if (!class_exists("UpDownPostCommentVotes"))
{
	...
	class UpDownPostCommentVotes
	{
		...
		public function load_vote_client_resources ()
		{
			if ( !is_admin () )
			{
				...
				wp_register_script ( 'updownupdown', plugins_url ( '/js/updownupdown.js', __FILE__), array ( 'jquery' ), '1.0' );
	...
	{
		add_options_page('UpDown Options', 'UpDown Plugin', 'manage_options', 'updown_plugin_menu_id', 'updown_options');
	}
	...
	function updown_options()
	{
		...
		if (isset ($_POST['Submit']))
		{
			...
			if (isset ($_POST['guest_allowed']) && $_POST['guest_allowed'] == "on")
			{
				...
				update_option (update_option ("updown_guest_allowed", "not allowed"));
